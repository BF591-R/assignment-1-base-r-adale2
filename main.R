...
less_than_zero <- function(x) {
  return(x<0) # returns a boolean (TRUE/FALSE) if the each individual value is less than 0
}
...
is_between <- function(x, a, b) {
  return(x>a & x<b) # returns a boolean if the value is greater than a AND less than b
}
...
#' [1] 1 2 3
rm_na <- function(x) {
  x <- x[!is.na(x)]  # removes the NA values and resaves the input without NAs
  return(x)
}
...
row_medians <- function(x) {
  return(apply(x,1,median)) # goes row by row through a matrix and finds the median of each row
}
...
summarize_rows <- function(x, fn, na.rm=FALSE) {
    return(apply(x,1,fn)) # allows you to specify the function of interest as an input to apply to each row of a matrix
}
...
# helper function to count how many values are greater than 1 and less than 5
is_between_1_and_5 <- function(x, na.rm=TRUE) {
  return(sum(x>1 & x<5))
}

# helper function to count how many values are NA
sum_na <- function(x, na.rm=TRUE) {
  return(sum(is.na(x)))
}

# helper function to count how many values are less than 0
sum_lt_zero <- function(x, na.rm=TRUE) {
  return(sum(x<0, na.rm=TRUE))
}

# applies functions to each row in a matrix and saving it to a dataframe
summarize_matrix <- function(x, na.rm=FALSE) {
    df = data.frame(
      ...
      min= apply(x,1,min),
      max= apply(x,1,max),
      num_lt_0= apply(x,1,sum_lt_zero),
      num_btw_1_and_5= apply(x,1,is_between_1_and_5),
      num_na= apply(x,1,sum_na)
      )
